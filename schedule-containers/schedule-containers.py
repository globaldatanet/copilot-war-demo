...
class ECS_Service:
    ...
    def start(self):
        # get ssm parameter
        param = ssm.get_parameter(Name=f"/ecs-shutdown-scheduler/{self.ecs_cluster_id}-{self.ecs_service_name}-TaskAmount")
    ...
    def _save_parameters(self, params):
        ssm.put_parameter(
            Name=f"/ecs-shutdown-scheduler/{self.ecs_cluster_id}-{self.ecs_service_name}-TaskAmount",
